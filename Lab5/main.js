...
export function openMoreForm(elemID) {
  ...
  currentMoreTeacherElement =
    document.getElementById(elemID);
  currentMoreTeacherElementID = elemID;
  for (let i = 0; i < randomUserApiMock.length; i++) {
    if (element.id == randomUserApiMock[i].id) {
      let nameElement =
        document.getElementById("more-name");
      nameElement.innerHTML =
        randomUserApiMock[i].full_name;
      let imgElement = document.getElementById("more-img");
      imgElement.src = randomUserApiMock[i].picture_large;
      let courseElement =
        document.getElementById("more-course");
      courseElement.innerHTML = randomUserApiMock[i].course;
      let cityCountryElement = document.getElementById(
        "more-city-country"
      );
      cityCountryElement.innerHTML =
        randomUserApiMock[i].city +
        ", " +
        randomUserApiMock[i].country;
      let ageSexElement =
        document.getElementById("more-age-sex");
      ageSexElement.innerHTML =
        randomUserApiMock[i].age +
        ", " +
        randomUserApiMock[i].gender;
      let emailElement =
        document.getElementById("more-email");
      emailElement.innerHTML = randomUserApiMock[i].email;
      let phoneElement =
        document.getElementById("more-phone");
      phoneElement.innerHTML = randomUserApiMock[i].phone;
      let starImg =
        document.getElementById("img-more-star");
      for (let g = 0; g < favoriteTeachers.length; g++) {
        if (
          favoriteTeachers[g].id == randomUserApiMock[i].id
        ) {
...
async function setGridStatisticsItems() {
  ...
  let arrayimg = Array.from(
    document.getElementsByClassName("img-teachers-orange")
  );
  arrayimg.forEach((el) => {
  ...
  let arrayh1 = Array.from(
    document.getElementsByClassName("teachers-h1")
  );
  arrayh1.forEach((el) => {
    let firstName = "";
    let lastName = "";
    ...
    for (
      ;
      i < randomUserApiMock[currentIndex].full_name.length;
      i++
    ) {
      if (
        randomUserApiMock[currentIndex].full_name[i] == " "
      ) {
        i++;
        break;
      }
  ...
  currentIndex = randomUserIndex;
  let arraydiv = Array.from(
    document.getElementsByClassName("teachers-grid-item")
  );
  arraydiv.forEach((el) => {
  ...
  currentIndex = randomUserIndex;
  let arraypfirst = Array.from(
    document.getElementsByClassName("teachers-p-first")
  );
  ...
  currentIndex = randomUserIndex;
  let arraypsecond = Array.from(
    document.getElementsByClassName("teachers-p-second")
  );
  ...
  let arrayName = Array.from(
    document.getElementsByName("td-name")
  );
  let arraySpecialty = Array.from(
    document.getElementsByName("td-specialty")
  );
  let arrayAge = Array.from(
    document.getElementsByName("td-age")
  );
  let arrayGender = Array.from(
    document.getElementsByName("td-gender")
  );
  let arrayNationality = Array.from(
    document.getElementsByName("td-nationality")
  );
  for (let i = 0; i < randomUserApiMock.length; i++) {
    ...
    statisticsFinalArrayToSort.push({
      name: randomUserMock[i].full_name,
      specialty: randomUserMock[i].course,
      age: randomUserMock[i].age,
      gender: randomUserMock[i].gender,
      nationality: randomUserMock[i].country,
    });
  }
  for (let i = 0; i < randomUserApiMock.length; i++)
...
async function setNextTeachers(arrayWithData) {
  if (
    currentDisplayedTeachersIndex * 10 >=
    arrayWithData.length
  )
  ...
  let arrayStar = Array.from(
    document.getElementsByClassName("teachers-div-relative")
  );
  let lastIndex = 0;
  for (
    let i = 0;
    i < 10 && currentIndex < arrayWithData.length;
    i++, currentIndex++
  ) {
    arrayimg[i].src =
      arrayWithData[currentIndex].picture_large;
    let firstName = "";
    let lastName = "";
    let c = 0;
    for (
      ;
      c < arrayWithData[currentIndex].full_name.length;
      c++
    ) {
    ...
    arraypfirst[i].innerHTML =
      arrayWithData[currentIndex].course;
    arraypsecond[i].innerHTML =
      arrayWithData[currentIndex].country;

    let isFavorite = false;
    for (let g = 0; g < favoriteTeachers.length; g++)
      if (
        arrayWithData[currentIndex].id ==
        favoriteTeachers[g].id
      ) {
  ...
  if (isFiltering && isSearching) {
    ...
    for (let i = 0; i < finalFilterArray.length; i++)
      for (let c = 0; c < finalSearchArray.length; c++)
        if (
          finalFilterArray[i].id == finalSearchArray[c].id
        ) {
...
export function addToFavorites() {
  for (let i = 0; i < randomUserApiMock.length; i++) {
    if (
      currentMoreTeacherElement.id ==
      randomUserApiMock[i].id
    ) {
      let isFavorite = false;
      let b = 0;
      for (; b < favoriteTeachers.length; b++)
        if (
          favoriteTeachers[b].id == randomUserApiMock[i].id
        ) {
          isFavorite = true;
          break;
        }
      if (!isFavorite) {
        let arrayimg = Array.from(
          document.getElementsByClassName(
            "img-favorites-orange"
          )
        );
        let arrayh1 = Array.from(
          document.getElementsByClassName("favorites-h1")
        );
        let arraypfirst = Array.from(
          document.getElementsByClassName(
            "favorites-p-first"
        ...
        let arraydiv = Array.from(
          document.getElementsByName("favorites-div")
        );
        for (let c = 0; c < arraydiv.length; c++)
          if (arraydiv[c].id == "") {
            arrayimg[c].src =
              randomUserApiMock[i].picture_large;
            let firstName = "";
            let lastName = "";
            let g = 0;
            for (
              ;
              g < randomUserApiMock[i].full_name.length;
              g++
            ) {
              if (
                randomUserApiMock[i].full_name[g] == " "
              ) {
                g++;
                break;
              }
        ...
        let arrayRelativeStars = Array.from(
          document.getElementsByClassName(
            "teachers-div-relative"
          )
        );
        let arrayTeachers = Array.from(
          document.getElementsByClassName(
            "teachers-grid-item"
          )
        );
        ...
        for (let c = 0; c < arrayFavorites.length; c++)
          if (
            ...
            indexForFavorites = c;
        for (
          let c = currentFavoritesFirstElement;
          c < favoriteTeachers.length;
          c++
        )
          if (
            favoriteTeachers[c].id ==
            currentMoreTeacherElement.id
          ) {
            if (c + 1 < favoriteTeachers.length)
              indexForFavoriteTeachers = c + 1;
            else indexForFavoriteTeachers = c;
            break;
          }
        ...
        for (let b = indexForFavorites; b < 5; b++) {
          for (let c = 0; c < randomUserApiMock.length; c++)
            if (
              randomUserApiMock[c].id ==
              favoriteTeachers[indexForFavoriteTeachers].id
            ) {
          ...
          for (
            ;
            g <
            randomUserApiMock[indexForRandomMock].full_name
              .length;
            g++
          ) {
            if (
              randomUserApiMock[indexForRandomMock]
                .full_name[g] == " "
            ) {
              g++;
              break;
          ...
          arraypfirst[b].innerHTML =
            randomUserApiMock[indexForRandomMock].course;
          let arraypsecond = Array.from(
            document.getElementsByClassName(
              "favorites-p-second"
            )
          );
          arraypsecond[b].innerHTML =
            randomUserApiMock[indexForRandomMock].country;
          let arraydiv = Array.from(
            document.getElementsByName("favorites-div")
          );
          arraydiv[b].id =
            randomUserApiMock[indexForRandomMock].id;
          if (
            indexForFavoriteTeachers + 1 <
            favoriteTeachers.length
          )
            indexForFavoriteTeachers =
              indexForFavoriteTeachers + 1;
        }
        ...
        for (
          let g = currentFavoritesLastElement - 1;
          g >= currentFavoritesFirstElement - 1;
          g--
        )
          if (g > favoriteTeachers.length - 1) {
            ...
            arrayimg[g - currentFavoritesFirstElement].src =
              "A.png";
            let arrayh1 = Array.from(
              document.getElementsByClassName(
                "favorites-h1"
            ...
            arrayh1[
              g - currentFavoritesFirstElement
            ].innerHTML = "";
            let arraypfirst = Array.from(
              document.getElementsByClassName(
                "favorites-p-first"
  ...
  let favoritesValue = document.getElementById(
    "only-favorites"
  ).checked;
  if (favoritesValue) {
    let favoritesCheckBox = document.getElementById(
      "only-favorites"
    );
...
export function previousFavorites() {
  if (currentFavoritesFirstElement >= 6) {
    ...
    let arraypfirst = Array.from(
      document.getElementsByClassName("favorites-p-first")
    );
    let arraypsecond = Array.from(
      document.getElementsByClassName("favorites-p-second")
    );
    for (let c = 0; c < arraydiv.length; c++) {
      let arrayimg = Array.from(
        document.getElementsByClassName(
    ...
    for (; g < favoriteTeachers.length; g++)
      for (let i = 0; i < randomUserApiMock.length; i++)
        if (
          randomUserApiMock[i].id == favoriteTeachers[g].id
        ) {
          for (let c = 0; c < arraydiv.length; c++)
            if (arraydiv[c].id == "") {
              let firstName = "";
...
export function onFilterChange() {
  ...
  if (sexValue == "All") sexForIf = "";
  else if (sexValue == "Male") sexForIf = "male";
  else if (sexValue == "Female") sexForIf = "female";
  else if (sexValue == "Other") sexForIf = "other";
  if (regionValue == "All") regionForIf = "";
  else if (regionValue == "Germany")
    regionForIf = "Germany";
  else if (regionValue == "United States")
    regionForIf = "United States";
  if (favoritesValue) withFavorites = true;
  else withFavorites = false;
  for (let i = 0; i < finalSearchArray.length; i++) {
    if (
      (finalSearchArray[i].age >= ageLowerLimit &&
        finalSearchArray[i].age <= ageUpperLimit) ||
      finalSearchArray[i].age == undefined
    )
      if (
        finalSearchArray[i].country.includes(regionForIf) ||
        finalSearchArray[i].country == undefined
      )
        if (
          finalSearchArray[i].gender.includes(sexForIf) ||
          finalSearchArray[i].gender == undefined
        )
  ...
  for (
    let i = 0;
    i < finalFilterArray.length && i < 10;
    i++, currentIndex++
  ) {
    let arrayimg = Array.from(
      document.getElementsByClassName("img-teachers-orange")
    ...
    if (!withFavorites)
      for (let g = 0; g < favoriteTeachers.length; g++)
        if (
          finalFilterArray[i].id == favoriteTeachers[g].id
        ) {
          arrayStar[i].style.visibility = "visible";
          break;
        }
  ...
  if (finalStatisticsArray.length != 0) {
    totalCount = Math.ceil(
      finalStatisticsArray.length / 10
    );
...
export function sortByName(addObject) {
  ...
  if (
    statisticsCurrentElementID != undefined &&
    statisticsCurrentElementID != thFirst.id
  ) {
    let currentStatisticsElement = document.getElementById(
      statisticsCurrentElementID
    );
    currentStatisticsElement.style =
      "border-bottom: solid 0px  #000; background-color: white;";
    let images =
      currentStatisticsElement.getElementsByTagName("img");
  ...
  for (let i = 0; i < finalStatisticsArray.length; i++)
    statisticsFinalArrayToSort.push(
      finalStatisticsArray[i]
    );
  statisticsCurrentElementID = thFirst.id;
  thFirst.style =
    "border-bottom: solid 3px  #000; background-color: lightgrey;";
  let arrayimg = Array.from(
    document.getElementsByClassName("statistics-arrow")
  );
  let arrayName = Array.from(
    document.getElementsByName("td-name")
  );
...
export function statisticsButton(elemID) {
  let firstButton = document.getElementById(
    "statistics-first-button"
  );
  let secondButton = document.getElementById(
    "statistics-second-button"
  );
  let thirdButton = document.getElementById(
    "statistics-third-button"
  );
  let fourthButton = document.getElementById(
    "statistics-fourth-button"
  );
  let lastButton = document.getElementById(
    "statistics-last-button"
  );
  let pointsButton = document.getElementById(
    "statistics-points"
  );
  if (elemID == "statistics-first-button") {
    ...
    thirdButton.innerHTML = (
      currentStatisticsPage + 2
    ).toString();
    fourthButton.innerHTML = (
      currentStatisticsPage + 3
    ).toString();
    ...
    currentStatisticsPage = parseFloat(
      secondButton.innerHTML
    );
    if (
      currentStatisticsPage != 2 &&
    ...
    if (
      ...
      thirdButton.innerHTML =
        currentStatisticsPage.toString();
    else thirdButton.innerHTML = (3).toString();
    if (
      currentStatisticsPage != 2 &&
      currentStatisticsPage != 3
    )
    ...
    else fourthButton.innerHTML = (4).toString();
    lastButton.innerHTML = "Last";
    pointsButton.innerHTML = "...";
    pointsButton.style =
      "  color: black; background-color: white;";
    ...
    if (totalCount > 5) {
      if (currentStatisticsPage <= totalCount - 2) {
        ...
        if (currentStatisticsPage >= totalCount - 2) {
          ...
          pointsButton.style =
            "  color: #0096FF; background-color: white; cursor: pointer;";
        }
        setStatisticsData();
    ...
    elemID == "statistics-last-button" ||
    elemID == "statistics-points"
  ) {
...
function checkStatisticsButton() {
  ...
  if (totalCount == 1) {
    ...
    totalCount > 5 &&
    currentStatisticsPage == totalCount - 1
  ) {
    secondButton.innerHTML = (totalCount - 3).toString();
    thirdButton.innerHTML = (totalCount - 2).toString();
    fourthButton.innerHTML = (totalCount - 1).toString();
...
function setStatisticsData() {
  ...
  for (
    let i = (currentStatisticsPage - 1) * 10;
    i < statisticsFinalArrayToSort.length &&
    i < currentStatisticsPage * 10;
    i++
  ) {
    ...
    arrayName[
      i - (currentStatisticsPage - 1) * 10
    ].innerHTML = statisticsFinalArrayToSort[i].name;
    arraySpecialty[
      i - (currentStatisticsPage - 1) * 10
    ].innerHTML = statisticsFinalArrayToSort[i].specialty;
    arrayAge[
      i - (currentStatisticsPage - 1) * 10
    ].innerHTML = statisticsFinalArrayToSort[i].age;
    arrayGender[
      i - (currentStatisticsPage - 1) * 10
    ].innerHTML = statisticsFinalArrayToSort[i].gender;
    arrayNationality[
      i - (currentStatisticsPage - 1) * 10
    ].innerHTML = statisticsFinalArrayToSort[i].nationality;
  }
  if (lastIndex != -1) {
...
export function searchOnChange() {
  finalSearchArray = [];
  let teacherSearch = document.getElementById(
    "teacher-search"
  );
  if (teacherSearch.value) {
    ...
    if (isNumeric(firstWord)) {
      ...
      for (let i = 0; i < randomUserApiMock.length; i++) {
        if (
          randomUserApiMock[i].full_name != undefined &&
          randomUserApiMock[i].note != undefined
        ) {
          ...
          if (firstWord != "")
            if (
              ...
              randomUserApiMock[i].note
                .toLowerCase()
                .includes(firstWord.toLowerCase()) ||
              randomUserApiMock[i].full_name.includes(
                firstWord
              ) ||
              randomUserApiMock[i].note.includes(firstWord)
            )
              firstWordIsOk = true;
          if (secondWord != "")
            if (
              randomUserApiMock[i].full_name
                .toLowerCase()
                .includes(secondWord.toLowerCase()) ||
              randomUserApiMock[i].note
                .toLowerCase()
                .includes(secondWord.toLowerCase()) ||
              randomUserApiMock[i].full_name.includes(
              ...
              randomUserApiMock[i].note.includes(secondWord)
            )
              secondWordIsOk = true;
          if (thirdWord != "")
            if (
              randomUserApiMock[i].full_name
                .toLowerCase()
                .includes(thirdWord.toLowerCase()) ||
              randomUserApiMock[i].note
                .toLowerCase()
                .includes(thirdWord.toLowerCase()) ||
              randomUserApiMock[i].full_name.includes(
              ...
              randomUserApiMock[i].note.includes(thirdWord)
            )
              thirdWordIsOk = true;
          if (
            firstWordIsOk ||
...
function searchOnChangeFilterWithAge(
  ageWord,
  firstWord,
  secondWord
) {
  for (let i = 0; i < randomUserApiMock.length; i++) {
    ...
    if (randomUserApiMock[i].age == ageWord) {
      ...
      if (
        firstWordIsOk ||
        secondWordIsOk ||
        randomUserApiMock[i].age == ageWord
      )
        finalSearchArray.push(randomUserApiMock[i]);
    }
  }
...
export function addNewTeacher() {
  let addTeacherName = document.getElementById(
    "add-teacher-name"
  );
  let addTeacherSpecialty = document.getElementById(
    "add-teacher-specialty"
  );
  let addTeacherCountry = document.getElementById(
    "add-teacher-country"
  );
  let addTeacherCity = document.getElementById(
    "add-teacher-city"
  );
  let addTeacherEmail = document.getElementById(
    "add-teacher-email"
  );
  let addTeacherPhone = document.getElementById(
    "add-teacher-phone"
  );
  let addTeacherDate = document.getElementById(
    "add-teacher-birthday"
  );
  let addTeacherSex = Array.from(
    document.getElementsByName("add-teacher-sex")
  );
  let addTeacherColor = document.getElementById(
    "add-teacher-color"
  );
  let addTeacherNotes = document.getElementById(
    "add-teacher-notes"
  );
  ...
  if (newObject[0].full_name != "")
    Validated = newObject.every(
      (obj) =>
        typeof obj.full_name === "string" &&
        obj.full_name[0] === obj.full_name[0].toUpperCase()
    );
  ...
  if (newObject[0].city != "")
    Validated = newObject.every(
      (obj) =>
        ...
        obj.city[0] === obj.city[0].toUpperCase()
    );
  else Validated = false;
  Validated = newObject.every((obj) =>
    numberValidation.test(obj.phone)
  );
  Validated = newObject.every((obj) =>
    emailValidation.test(obj.email)
  );
  if (
    newObject[0].gender == "" ||
    newObject[0].gender == undefined
  )
    Validated = false;
...
function validateObjects() {
  ...
  Validated = randomUserApiMock.every(
    (obj) =>
      typeof obj.full_name === "string" &&
      obj.full_name[0] === obj.full_name[0].toUpperCase()
  );
  Validated = randomUserApiMock.every(
    (obj) =>
      ...
      obj.gender[0] === obj.gender[0].toUpperCase()
  );
  Validated = randomUserApiMock.every(
    (obj) =>
      typeof obj.note === "string" &&
      obj.note[0] === obj.note[0].toUpperCase()
  );
  Validated = randomUserApiMock.every(
    (obj) =>
      typeof obj.state === "string" &&
      obj.state[0] === obj.state[0].toUpperCase()
  );
  Validated = randomUserApiMock.every(
    (obj) =>
      typeof obj.city === "string" &&
  ...
  Validated = randomUserApiMock.every(
    (obj) =>
      ...
      obj.country[0] === obj.country[0].toUpperCase()
  );
  Validated = randomUserApiMock.every(
    (obj) => typeof obj.age === "number"
  );
  Validated = randomUserApiMock.every((obj) =>
    numberValidation.test(obj.phone)
  );
  Validated = randomUserApiMock.every((obj) =>
    emailValidation.test(obj.email)
...
function makeCourse() {
  ...
  if (randomNumber >= 0 && randomNumber < 1) return "Math";
  else if (randomNumber >= 1 && randomNumber < 2)
    return "Chemistry";
  else if (randomNumber >= 2 && randomNumber < 3)
    return "Art";
  else if (randomNumber == 3) return "English";
}
    ...
    .then((data) => {
      for (let i = 0; i < data.results.length; i++) {
        ...
        newObject.coordinates =
          data.results[i].location.coordinates;
        newObject.timezone =
          data.results[i].location.timezone;
